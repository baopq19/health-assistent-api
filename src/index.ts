...
server.get("/auth/login", async (req, res) => {
  const query = req.query as
    | { returnTo?: string; screenHint?: string }
    | undefined;
...
server.get("/auth/callback", async (req, res) => {
  ...
  if (query?.error) {
    ...
    if (query.error_description)
      url.searchParams.set("auth_error_description", query.error_description);
  ...
  if (!code)
    return res
      .status(400)
      .send({ ok: false, error: "Missing `code` query param" });

  const webReq = toWebRequest(req);
  const result = await authService.handleCallback(webReq, new Response(), {
    code,
    state: query?.state,
  } as any);
...
server.get("/auth/logout", async (req, res) => {
  ...
  if ((auth as any)?.user && (auth as any)?.sessionId) {
    const signOut = await authService.signOut((auth as any).sessionId, {
      returnTo,
    } as any);
    const logoutUrl = (signOut as any)?.logoutUrl as string | undefined;
    const clearCookieHeader = (signOut as any)?.clearCookieHeader as
      | string
      | undefined;
